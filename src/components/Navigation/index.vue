<template>
  <div class="navigation">
    <el-menu @select="handleOpen" background-color="#545c64" text-color="#fff" ref="menu"
      ...
      <el-submenu :index="item.node_name" v-for="(item, index) in _tree" :key="index">
        ...
        <el-menu-item :index="`/${item2.control_name}/${item2.action_name}/${item2.node_name}`"
          v-for="(item2, index2) in item.son" :key="index2">{{ item2.node_name }}</el-menu-item>
      </el-submenu>
    </el-menu>
  </div>
</template>
